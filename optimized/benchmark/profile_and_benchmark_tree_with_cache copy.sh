...
#
# TEMPORARY MODIFICATION FOR CRASH ANALYSIS:
# - All builds are configured with Debug mode, AddressSanitizer, and UndefinedBehaviorSanitizer
# - This will help identify the exact cause of the A2Q cache delete operation crash
# - These debug settings should be reverted to Release mode after crash analysis is complete
#
# Features:
...
build_cache_configuration() {
    ...
    # Define optimization flags for Debug builds (temporarily for crash analysis)
    local DEBUG_OPTS="-g3 -O0 -ggdb3 -fno-omit-frame-pointer -fsanitize=address -fsanitize=undefined"

    # Configure and build based on type
    if [ "$config_type" = "non_concurrent_default" ]; then
        echo "Building with cache + non_concurrent_default (DEBUG MODE FOR CRASH ANALYSIS) ..."
        ...
        echo "Building with cache + non_concurrent_relaxed (DEBUG MODE FOR CRASH ANALYSIS) ..."
    ...
    # LRU Specific
        ...
        echo "Building with cache + non_concurrent_lru_metadata_update_in_order (DEBUG MODE FOR CRASH ANALYSIS) ..."
        ...
        echo "Building with cache + non_concurrent_lru_metadata_update_in_order_and_relaxed (DEBUG MODE FOR CRASH ANALYSIS) ..."
    ...
    # A2Q Specific
        ...
        echo "Building with cache + non_concurrent_a2q_ghost_q_enabled (DEBUG MODE FOR CRASH ANALYSIS) ..."
        ...
        echo "Building with cache + non_concurrent_a2q_ghost_q_enabled_and_relaxed (DEBUG MODE FOR CRASH ANALYSIS) ..."
    ...
    # default
        ...
        echo "Building with cache + non_concurrent (DEBUG MODE FOR CRASH ANALYSIS) ..."
        cmake .. -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_FLAGS="-D__TREE_WITH_CACHE__ -D__CACHE_COUNTERS__ $DEBUG_OPTS"
    fi
