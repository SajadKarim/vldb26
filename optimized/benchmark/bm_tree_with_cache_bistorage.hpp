...
namespace bm_tree_with_cache_bistorage {
...
/**
 * @brief Macro to instantiate and run BiStorage benchmark
 * 
 ...
 */
#define INSTANTIATE_AND_RUN_BISTORAGE(CACHE_OBJ, CACHE_TYPE, STORAGE_TYPE, PRIMARY_PATH, SECONDARY_PATH) \
    do { \
        using T = bm_tree_with_cache_real::BPlusStoreTraits<int, int, TYPE_UID::DATA_NODE_INT_INT, \
        ...
        \
        bm_tree_with_cache_real::run_real_cache_benchmark<typename T::StoreType, int, int>( \
            ptrTree, cache_type, storage_name, "int", "int", degree, records, \
            operation, logger, runs, output_dir, threads, config_name, \
            cache_size_percentage, cache_page_limit); \
    } while(0)

/**
 * @brief Main function to test BiStorage with shell parameters
 * 
 ...
 * - LRU, A2Q, CLOCK
 */
void test_bistorage_with_shell_parameters(
...
{
    std::cout << "\n=== Testing B+ Tree with " << cache_type << " Cache and BiStorage ===" << std::endl;
    ...
    std::cout << "Operation: " << operation << ", Degree: " << degree << ", Records: " << records << std::endl;
    ...
    std::string csv_filename = CSVLogger::generate_filename("benchmark_bistorage");
    CSVLogger logger(csv_filename, output_dir);
    logger.write_header();
    std::string full_path = output_dir.empty() ? csv_filename : output_dir + "/" + csv_filename;
    std::cout << "Logging BiStorage results to: " << full_path << std::endl;
    ...
    if (primary_storage_type == "VolatileStorage" && secondary_storage_type == "VolatileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_Volatile_Volatile, "", "");
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_Volatile_Volatile, "", "");
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_Volatile_Volatile, "", "");
        } else {
        ...
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_Volatile_PMem, "", PMEM_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_Volatile_PMem, "", PMEM_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_Volatile_PMem, "", PMEM_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "VolatileStorage" && secondary_storage_type == "FileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_Volatile_File, "", FILE_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_Volatile_File, "", FILE_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_Volatile_File, "", FILE_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "PMemStorage" && secondary_storage_type == "VolatileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_PMem_Volatile, PMEM_STORAGE_PATH, "");
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_PMem_Volatile, PMEM_STORAGE_PATH, "");
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_PMem_Volatile, PMEM_STORAGE_PATH, "");
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "PMemStorage" && secondary_storage_type == "PMemStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_PMem_PMem, PMEM_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_PMem_PMem, PMEM_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_PMem_PMem, PMEM_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "PMemStorage" && secondary_storage_type == "FileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_PMem_File, PMEM_STORAGE_PATH, FILE_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_PMem_File, PMEM_STORAGE_PATH, FILE_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_PMem_File, PMEM_STORAGE_PATH, FILE_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "FileStorage" && secondary_storage_type == "VolatileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_File_Volatile, FILE_STORAGE_PATH, "");
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_File_Volatile, FILE_STORAGE_PATH, "");
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_File_Volatile, FILE_STORAGE_PATH, "");
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "FileStorage" && secondary_storage_type == "PMemStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_File_PMem, FILE_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_File_PMem, FILE_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_File_PMem, FILE_STORAGE_PATH, PMEM_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else if (primary_storage_type == "FileStorage" && secondary_storage_type == "FileStorage") {
        if (cache_type == "LRU") {
            INSTANTIATE_AND_RUN_BISTORAGE(LRUCacheObject, LRUCache, BiStorage_File_File, FILE_STORAGE_PATH, FILE_STORAGE_PATH);
        } else if (cache_type == "A2Q") {
            INSTANTIATE_AND_RUN_BISTORAGE(A2QCacheObject, A2QCache, BiStorage_File_File, FILE_STORAGE_PATH, FILE_STORAGE_PATH);
        } else if (cache_type == "CLOCK") {
            INSTANTIATE_AND_RUN_BISTORAGE(CLOCKCacheObject, CLOCKCache, BiStorage_File_File, FILE_STORAGE_PATH, FILE_STORAGE_PATH);
        } else {
            std::cerr << "Unsupported cache type: " << cache_type << std::endl;
        }
    } else {
    ...
    std::cout << "\nBiStorage benchmark completed!" << std::endl;
}
