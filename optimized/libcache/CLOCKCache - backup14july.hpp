...
public:
	...
	CacheErrorCode updateObjectsAccessMetadata(std::vector<ObjectTypePtr>& vtObjects, int level, std::vector<int> &vt)
	{
...

		{
			...
			cv.wait(lock_cache, [&] { return m_vtClockQ.size() < m_nCacheCapacity; });
	...
	CacheErrorCode remove(const ObjectTypePtr ptrObject)
	{
		...
		ASSERT(ptrObject->m_ptrCoreObject);
...
private:
	...
	inline void flushItemsToStorage(std::vector<int>& vtEmptySlots)
	{
		...
		*/
		...
		for (; nIdx < nQSiz; nIdx++)
		{
			auto& obj = m_vtClockQ[nIdx].first;
			...
			if (obj->m_nPositionInCLOCK == -1)
			{
				...
				if (obj)
				{
					...
					m_vtClockBuffer[nSlotIdx] = obj;	// use atomics!!!!
					obj->m_nPositionInCLOCK = nSlotIdx;
					obj->reftofirstiteminpair = &m_vtClockBufferWeight[nSlotIdx];
				}
				else
				{
				ASSERT(!obj);
			...
			if (obj)
			{
				*obj->reftofirstiteminpair = m_vtClockQ[nIdx].second;
			}
			else
			{
				ASSERT(!obj);
