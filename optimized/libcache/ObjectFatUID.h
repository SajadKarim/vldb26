...
public:
	...
	inline bool isPersistedObject() const
	{
		return m_uid.m_nMediaType >= 2 ? true : false;
	}
...
public:
	...
	void createUIDFromVolatilePointer(uint8_t nType, uintptr_t ptr)
	{
		m_uid.m_nType = nType;
		m_uid.m_nMediaType = Volatile;
		m_uid.FATPOINTER.m_ptrVolatile = ptr;
	}

	static void createAddressFromDRAMCacheCounter(ObjectFatUID& uid, uint8_t nType, size_t nOffset, uint32_t nSize)
	{
		if (nOffset > 600000000)
		{
...
public:
	...
	ObjectFatUID(size_t n) noexcept
	{
		m_uid.m_nType = 0;
		m_uid.m_nMediaType = 0;
		m_uid.FATPOINTER.m_ptrFile.m_nOffset  = n;
		m_uid.FATPOINTER.m_ptrFile.m_nSize = INT32_MAX;
	}
	...
	std::string toString() const
	{
		...
		case Volatile:
			...
			break;
		case DRAM:
			szData.append(std::to_string(m_uid.m_nType));
			szData.append(std::to_string(m_uid.m_nMediaType));
			szData.append("D:");
