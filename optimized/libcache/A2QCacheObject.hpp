...
class A2QCacheObject 
{
	typedef A2QCacheObject<Traits> SelfType;
	typedef A2QCacheObject<Traits>* SelfTypePtr;
...
public:
	...
	A2QCacheObject(uint8_t nCoreObjectType, void* ptrCoreObject, uint8_t nQueueType)
		: m_bDirty(true)
#ifdef __CONCURRENT__
		, m_nUseCounter(0)
#endif //__CONCURRENT__
	...
	A2QCacheObject(uint16_t nDegree, const ObjectFatUID& uidObject, const char* szBuffer, uint32_t nDataLen, uint16_t nBlockSize)
		: m_bDirty(false)
#ifdef __CONCURRENT__
		, m_nUseCounter(0)
#endif //__CONCURRENT__
