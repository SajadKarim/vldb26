...
public:
	...
	MMapStorage(uint32_t nBlockSize, uint64_t nStorageSize, const std::string& stFilename)
		...
		, m_nStorageSize(nStorageSize)
	{
		...
		m_tBackgroundFlush = std::thread(&MMapStorage::backgroundFlushLoop, this);
	}
...
//#endif
				...
				for (auto& b : m_vtWrites)
				{
					memcpy(m_pMappedStorage + b.offset, b.buffer, b.size);
				...
				//msync(m_pMappedStorage + b.offset, b.size, MS_SYNC);
