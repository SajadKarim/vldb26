...
// Template specializations for creating BPlusStore instances with correct storage parameters

// Primary template - default case (should not be used)
template<typename BPlusStoreType>
std::unique_ptr<BPlusStoreType> create_bplus_store(size_t degree, size_t cache_size, size_t page_size, size_t memory_size) {
    static_assert(sizeof(BPlusStoreType) == 0, "Unsupported BPlusStore type");
    return nullptr;
}

// VolatileStorage specializations
template<>
